...
int pruebaCoord() {
	...
	cout << "EINA: starting position(Lat, Long):  (" << Lat << "," << Long
			<< ")" << endl;
	...
	cout << "EINA: calculated Lat, Long position(Lat, Long):  (" << Lat << ","
			<< Long << ")" << endl;
	//----------------------------------------------------------------
	...
	cout << "detalle_ArtePublico_97 en Torrero (Lat, Long):  (" << Lat << ","
			<< Long << ")" << endl;
	string cmd(
			"google-chrome https://www.google.com/maps/place/" + to_string(Lat)
					+ "," + to_string(Long));
...
int main() {
	 ...
	 int numMon = obtenerDatos(monumentos,"datos/arte.json");
	 int numRes = obtenerDatos(restaurantes,"datos/restaurantes.json");
