...
bool messageParser(string buffer, array<string, 5> &url, array<string, 5>& nom,
		string delimiter) {
	int i = 0;
	size_t pos = 0;
	string token;
...
int main(int argc, char* argv[]) {
	...
	string posibilidades[6] = { "titulo", "URL", "descripcion", "categoria",
			"fecha", "icono" };
	string termino[6];
	bool correcto[6];
	int contador = 0;
	...
	while (!out) {
		...
		if (contador == 0) {
			cerr << "No hay suficientes términos para hacer la búsqueda: "
					<< strerror(errno) << endl;
			exit(1);
		} else if (contador > 5) {
			...
			if (aux != "Ningun resultado") {
				...
				for (int i = 0; i < URL.size(); i++) {
					if (URL[i] != "") {
						...
						if (resCall != 0) {
							cerr
									<< "Ha habido algún problema al abrir el navegador"
									<< endl;
							return 1;
						}
					}
				...
				if (eleccion != -1) {
					string busqueda;
					cout<< "Elija una URL para obtener el restaurante mas cercano al monumento:"
							<< endl;
					...
					int num;
					cin >> num;
					while (num < 0 || num > cuenta) {
						cout << "Error, vuelva a elegir un valor entre 1 y "
								<< cuenta << ": ";
						cin >> num;
					}
					message = to_string(num).c_str();
					...
					cerr << "Esperamos a recibir el restaurante..." << endl;
					...
					lon = restaurante.substr(
							restaurante.find_first_of(';') + 1);

					string cmd(
							"firefox https://www.google.com/maps/place/" + lat
									+ "," + lon);
					//"system" requiere un "char *", que es lo que nos da el operador "c_str()" de la clase string de C++
					int resCall = system(cmd.c_str());
					if (resCall != 0) {
						cerr << "Ha habido algún problema al abrir el navegador"
								<< endl;
						return 1;
					}
				}
